...
//   } catch(err) { return res.status(500).json({ success:false, message:'Server error.' }); }
// };
// const getDailyWorkSummary = async (req, res) => {
//   try {
//     return res.status(200).json({
//       success: true,
//       message: "Daily work summary fetched successfully",
//       data: []
//     });
//   } catch (err) {
//     console.error("getDailyWorkSummary error:", err);
...
const createAccount = async ({
  ...
  sendWelcomeEmail({
    ...
    console.error(`Welcome email failed for ${officialEmail}:`, err.message);
  });
...
const addEmployee = async (req, res) => {
  try {
    ...
    if (role === 'hr') {
      ...
      if (existingHR) {
        return res.status(409).json({
          success: false,
          message: 'Only one HR account is allowed.'
        });
      }
    }
    ...
    const empIdExists = await Employee.findOne({
...
const bulkUploadEmployees = async (req, res) => {
  try {
    ...
    const successRows = [];
    const failedRows = [...invalid];

    for (const emp of valid) {
      try {
        await createAccount(emp);
        successRows.push(emp.employeeId);
      } catch (err) {
        failedRows.push({
          data: emp,
          errors: [err.message]
