...
// controllers/leaveController.js — 2-step approval (TL → HR). Fully Fixed for Frontend & Email Compatibility.
const Leave    = require('../models/Leave');
const Employee = require('../models/Employee');
const Project  = require('../models/Project');
const User     = require('../models/User');
...
const applyLeave = async (req, res) => {
  try {
    ...
    let { fromDate, toDate, from_date, to_date, reason } = req.body;
    
    const finalFromDate = fromDate || from_date;
    const finalToDate = toDate || to_date;

    if (!finalFromDate || !finalToDate || !reason)
      return res.status(400).json({ success: false, message: 'All fields are required.' });
    ...
    // Tech Lead leaves skip TL review — go straight to HR
    const leave = await Leave.create({
      employee:           employee._id,
    ...
    (async () => {
      try {
        const hr = await getHR();
        const hrEmailAddress = hr?.email || "admin@company.com"; 
        ...
        if (isTechLead) {
          ...
          if (employee.currentProject) {
            ...
            if (project?.createdBy?.email) {
              techLeadEmails = [project.createdBy.email];
            }
          }

          if (techLeadEmails.length === 0) {
            console.log("⚠️ Tech Lead email not defined for this employee project, routing notification to HR.");
            techLeadEmails = [hrEmailAddress]; 
          }
        ...
        console.error('⚠️ Leave notification email sent fail but record saved:', err.message); 
      }
    })();
      ...
      (async () => {
        try {
          ...
          const tlName = reviewerEmp
            ? `${reviewerEmp.firstName} ${reviewerEmp.lastName}`
            : reviewer.email;
          await sendLeaveToHRAfterTLApproval({ hrEmail: hrEmailAddress, employee: leave.employee, leave, techLeadName: tlName });
        } catch (err) { console.error('HR notification error:', err.message); }
      })();
    }
